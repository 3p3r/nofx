...
namespace nofx
{
    namespace OfMatrix4x4
    {
        ...
        public:
            ...
            ofMatrix4x4* GetWrapped() const { return internal_.get(); };
            void SetWrapped(ofMatrix4x4* n)  { internal_.reset(n); };
            void SetWrapped(ofMatrix4x4& n)  { if (!internal_) { internal_.reset(new ofMatrix4x4()); } internal_.get()->set(n); };
            static const Persistent<Function>& Factory() { return constructor; }
        private:
            OfMatrix4x4Wrap();
            OfMatrix4x4Wrap(ofMatrix4x4*);
            ~OfMatrix4x4Wrap() {};
            ...
            //Js ctor, can be used inside the class itself to construct ofMatrix4x4
            static Persistent<Function> constructor;
            static NAN_METHOD(New);
            
            //Pointer to internal object
            std::shared_ptr<ofMatrix4x4> internal_;
        }; // !class OfMatrix4x4Wrap
    } //!namespace OfMatrix4x4
} // !namespace nofx
